...
def run_voice_test():
    ...
    text_slow = "Hello! This is the new, even slower speed at negative ten percent, specifically designed for level three students."
    ...
    # ---------------------------------------------------------
    # TEST 2: Slow Speed (-10%)
    # ---------------------------------------------------------
    print("\n▶️ [TEST 2] Generating and playing SLOW speed (-10%)")
    duration = vm.speak(text_slow, "test_slow", rate="-10%")
    time.sleep(duration / 1000.0 + 0.5)
    
    # ---------------------------------------------------------
    # TEST 3: Name Concatenation at Slow Speed (-10%)
    # ---------------------------------------------------------
    print("\n▶️ [TEST 3] Testing Name Concatenation System at SLOW speed (-10%)")
    template = "You did an amazing job, {name}! Keep up the great work!"
    duration = vm.speak_with_name(template, "Alice", "test_name_slow", rate="-10%")
    time.sleep(duration / 1000.0 + 0.5)
