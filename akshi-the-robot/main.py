...
def head_pose_from_mesh(landmarks, W, H, focal=800.0):
    ...
    idx = {"nose_tip": 1, "left_eye_out": 33, "right_eye_out": 263,
           "mouth_left": 61, "mouth_right": 291, "chin": 199}
    pts2d = np.array([
...
def get_face_center(frame_rgb, out_w, out_h):
    if PROC_SCALE != 1.0:
        small = cv2.resize(frame_rgb, (int(out_w*PROC_SCALE),
                           int(out_h*PROC_SCALE)), interpolation=cv2.INTER_AREA)
    else:
        small = frame_rgb
...
if not pi.connected:
    raise RuntimeError(
        "pigpio daemon not running. Start it: sudo systemctl start pigpiod")
...
# << hysteresis state (stop inside lock radius; move outside reacquire radius)
moving = False

# ===================== LOOP =====================
running = True
signal.signal(
    signal.SIGINT, lambda *_: globals().__setitem__('running', False))
...
try:
    while running:
        ...
        if center is not None:
            ...
            if DRAW_RADII:
                cv2.circle(flipped, (FRAME_W//2, FRAME_H//2),
                           LOCK_RADIUS_PIX, (0, 255, 0), 1)
                cv2.circle(flipped, (FRAME_W//2, FRAME_H//2),
                           REACQUIRE_RADIUS_PIX, (0, 165, 255), 1)
            ...
            if moving and advice["mode"] != "chassis":
                ...
                target_yaw = yaw_deg * SMOOTH_ALPHA_CMD + \
                    (yaw_deg + cmd_yaw) * (1 - SMOOTH_ALPHA_CMD)
                target_pitch = pitch_deg * SMOOTH_ALPHA_CMD + \
                    (pitch_deg + cmd_pitch) * (1 - SMOOTH_ALPHA_CMD)
                dyy = clamp(target_yaw - yaw_deg,   -
                            MAX_DELTA_YAW,   +MAX_DELTA_YAW)
                dpp = clamp(target_pitch - pitch_deg, -
                            MAX_DELTA_PITCH, +MAX_DELTA_PITCH)

                if abs(dyy) >= MIN_STEP_YAW_DEG:
                    yaw_deg = servo_yaw.set_angle(yaw_deg - dyy)
            ...
            cv2.circle(flipped, (int(ax_f), int(ay_f)), 4,
                       (0, 255, 0) if moving else (0, 200, 255), -1)
        else:
