"""
🌿 Evaluate Game Widget — "Akshi's Enchanted Forest"
=====================================================
...
class EvaluateGameWidget(QWidget):
    ...
    def _generate_scene(self, w, h):
        """Pre-generate flowers and butterflies."""
        self.flowers = []
        for _ in range(14):
    ...
    def paintEvent(self, event):
        ...
        # Dim overlay when highlighting a specific option
    ...
    def _draw_single_card(self, p, x, y, w, h, opt, num, ground_y=None, skip_post=False):
        ...
        # Image (uses full card height minus padding for badge)
        pixmap = opt.get("pixmap")
        if pixmap:
            img_margin = 14
            img_area_top = y + img_margin
            img_area_h = h - 50  # leave room for badge at bottom
            max_img = min(int(w - img_margin * 2), int(img_area_h))
            scaled = pixmap.scaled(max_img, max_img, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            img_x = x + (w - scaled.width()) / 2
            img_y = img_area_top + (img_area_h - scaled.height()) / 2
