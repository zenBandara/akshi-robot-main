...
pyenv activate ginglu
cd /path/to/ginglu-robot-main
...
|---|---|---|
| `dummy_robot.py` | `ginglu-the-robot/maincopy.py` | Fake camera + WebSocket server on `:8765`. Streams synthetic frames, reads IPC JSON commands, responds to the analysis server. |
...
| `dummy_firebase.py` | `ginglu-the-robot/firebase_request.py` + `server/firebase_operations.py` | Uses real Firebase credentials (if present) or gracefully degrades. |
